...
::  Removes log folder
    rmdir /Q /S "%~dp0\%YYYYMMDD%-Logs"
                            ...
                            ::    Powershell -NoProfile -ExecutionPolicy Bypass -Command "& {Install-Module -Name PSWindowsUpdate; Get-WindowsUpdate -AcceptAll -Install; Get-WUHistory | out-file -FilePath %~dp0\%YYYYMMDD%-Logs\WindowsUpdateHistory.txt}"

                            :: Checks if Windows Update Logs have been generated. If so, will exit.
                            ::    IF EXIST "%~dp0\%YYYYMMDD%-Logs\WindowsUpdateHistory.txt" EXIT